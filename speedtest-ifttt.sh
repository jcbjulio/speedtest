...
###########################################################################
currentDate=$(date)
echo "Starting SpeedTest $currentDate"
...
# Send to IFTTT
secret_key=$IFTTT_KEY
value1=`echo $server_ping | cut -d" " -f1`
value2=`echo $download | cut -d" " -f1`
value3=`echo $upload | cut -d" " -f1`
echo "Result: ping=$value1, down=$value2, up=$value3"
if test -n "$secret_key"; then
